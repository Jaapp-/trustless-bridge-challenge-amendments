...
export function loadAccountBlock(slice: Slice): AccountBlock {
    if (slice.remainingBits >= 4 && slice.preloadUint(4) == 0x5) {
        slice.loadUint(4);
        let account_addr: BitString = slice.loadBits(256);
        let transactions: HashmapAug<Transaction, CurrencyCollection> = loadHashmapAug<Transaction, CurrencyCollection>(
            slice,
            64,
            (slice: Slice) => {
                let slice1 = slice.loadRef().beginParse(true);
...
export function loadShardAccountBlocks(slice: Slice): ShardAccountBlocks {
    let anon0: Dictionary<bigint, { value: AccountBlock; extra: CurrencyCollection }> = Dictionary.load(
        Dictionary.Keys.BigUint(256),
        {
            serialize: () => {
                throw new Error('Not implemented');
...
export function loadShardFees(slice: Slice): ShardFees {
    let anon0: Dictionary<bigint, { value: ShardFeeCreated; extra: ShardFeeCreated }> = Dictionary.load(
        Dictionary.Keys.BigUint(96),
        {
            serialize: () => {
                throw new Error('Not implemented');
